...
public boolean superDispatchTouchEvent(MotionEvent event) {
            // <2>调用分类（ViewGroup）的dispatchTouchEvent()方法
...
4. 第四步：View.dispatchTouchEvent()方法：
``` java 
/**
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        // <2>否则，调用View.onTouchEvent()方法
        return onTouchEvent(event);
    }

```
即，像下面的例子一样，如果给当前View设置了TouchListener，并且回调方法onTouch()返回true，并且当前控件是enable的，则，时间传递终止，也就是说，在onTouch()里处理点击事件
```java
//手动调用设置
